...
## 3
#b
x3 <- seq(0, 1, length.out = 300)

error_func3 <- function(v){
  abs(pbeta(.05,v[1],v[2]) - .02) + abs((1 - pbeta(.9, v[1], v[2])) - .12
        + abs((pbeta(.5, v[1], v[2]) - pbeta(.5, v[1], v[2])) - .8))
}
?qbeta
nlm_out3 <- nlm(error_func3, c(2,2), stepmax = 1, iterlim = 1000, gradtol = .000000001)
nlm_out3


plot(x3, dbeta(x3, 2.3, 4.28), type = "l")

plot(x3, dbeta(x3, 2.370455, 5.800109), type = "l")
...
nlm_out4 <- nlm(error_func, c(2,2), stepmax = 1, iterlim = 1000, gradtol = .000000001)
nlm_out4


x4 <- seq(.0001,.9999, length.out = 1000)
plot(x4, dbeta(x4, nlm_out4$est[1], nlm_out4$est[2]), type = "l", xlab = "y",
     ylab = expression(paste("p( y| ", alpha, " , ", beta, " )")),
     main = expression(paste("Beta distribution with ", alpha, " = 0.7920481, ", 
                             beta, " = 1.4748258")))
...
#d
error_func5 <- function(v){
 return( abs((pgamma(15,v[1],v[2]) + pgamma(25, v[1], v[2], lower.tail = FALSE)) - .20) + 
  abs((pgamma(10,v[1],v[2]) + pgamma(35, v[1], v[2], lower.tail = FALSE)) - .05) + 
  abs((pgamma(1,v[1],v[2]) + pgamma(50, v[1], v[2], lower.tail = FALSE)) - .01) )
}

nlm_out5 <- nlm(error_func5, c(20 , 1), stepmax = .3, iterlim = 10000, gradtol = .0000000001)
nlm_out5

x5 <- seq(.0001,60, length.out = 1000)
plot(x5, dgamma(x5, 21.753974  ,1.075465), type = "l")

#e
set.seed(2015)
lambd <- rgamma(1, 21.753974  ,1.075465)
samp1 <- rpois(20, lambd)

#f
par(mfrow = c(1,2))
hist(samp1, breaks = seq(min(samp1), max(samp1), length.out = 8), freq = FALSE,
     main = "Histogram of Sample", xlab = "y")
abline(v = lambd)
plot(density(samp1), main = "Density Plot of Sample")
abline(v = lambd)

#g
sum(samp1)

likeli.pois <- function(lambdas) prod((lambdas^(samp1))*(exp(- lambdas))/factorial(samp1))
likeli.pois.vec <- Vectorize(likeli.pois)
like_vals <- likeli.pois.vec(x5)

par(mfrow = c(1, 3))
plot(x5, like_vals, type = "l", main = "Likelihood")
plot(x5, dgamma(x5, 21.753974  ,1.075465), type = "l", main = "Prior")
plot(x5, dgamma(x5, 307.754, 21.075465), type = "l", main = "Posterior")

#h
c( qgamma(.05, 307.754, 21.075465), qgamma(.95, 307.754, 21.075465, lower.tail = FALSE))
?qgamma
options(digits = 16)
qgamma(.05, 307.754, 21.075465)

#i
pgamma(20, 307.754, 21.075465)  - pgamma(10, 307.754, 21.075465)

#j
pgamma(5, 307.754, 21.075465) 

#k
## 7
x7 <- seq(-3, 3, length.out = 5000)
norm_samp <- rnorm(500000)
par(mfrow = c(1,3))
hist(norm_samp, freq = FALSE, ylim = c(0, .5), 
     breaks = seq(min(norm_samp), max(norm_samp), length.out = 12),
     main = "10 Bins")
lines(x7, dnorm(x7), type ="l", col = "red")
hist(norm_samp, freq = FALSE, ylim = c(0, .5), 
     breaks = seq(min(norm_samp), max(norm_samp), length.out = 22),
     main = "20 Bins")
lines(x7, dnorm(x7), type ="l", col = "red")
hist(norm_samp, freq = FALSE, ylim = c(0, .5), 
     breaks = seq(min(norm_samp), max(norm_samp), length.out = 42),
     main = "40 Bins")
lines(x7, dnorm(x7), type ="l", col = "red")
...
<<plot3, fig.width=6, fig.height=2.6, fig.align='center', echo=FALSE, results='hide', cache=TRUE>>=
par(mfrow = c(1,2))
plot(x3, dbeta(x3, 2.3, 4.28), type = "l", main = "Author's", xlab = "Success Rate",
     ylab = "Probability")
plot(x3, dbeta(x3, 2.370455, 5.800109), type = "l", main = "Mine", xlab = "Success Rate",
     ylab = "Probability")
@
...
<<plot4, fig.width=4.5, fig.height=3.5, fig.align='center', echo=FALSE, results='hide', cache=TRUE>>=
plot(x4, dbeta(x4, nlm_out4$est[1], nlm_out4$est[2]), type = "l", xlab = "y",
     ylab = expression(paste("p( y| ", alpha, " , ", beta, " )")),
     main = expression(paste("Beta dist. with ", alpha, " = 0.7920481, ", 
                             beta, " = 1.4748258")))
@
...
<<plot5f, fig.width=6, fig.height=3.5, fig.align='center', echo=FALSE, results='hide', cache=TRUE>>=
par(mfrow = c(1,2))
hist(samp1, breaks = seq(min(samp1), max(samp1), length.out = 8), freq = FALSE,
     main = "Histogram of Sample", xlab = "y")
abline(v = lambd)
...
<<plot7, fig.width=6, fig.height=3.5, fig.align='center', echo=FALSE, results='hide', cache=TRUE>>=

par(mfrow = c(1,3))
hist(norm_samp, freq = FALSE, ylim = c(0, .42), 
     breaks = seq(min(norm_samp), max(norm_samp), length.out = 12),
     main = "10 Bins", xlab = "y")
lines(x7, dnorm(x7), type ="l", col = "red")
hist(norm_samp, freq = FALSE, ylim = c(0, .42), 
